...
=item B<a[1|2|3|I<config>[,I<arg>] ... ] I<archive> [I<folder/>] I<file> ...>
...
=item B<c[1|2|3|I<config>[,I<arg>] ... ] I<archive> [I<folder/>] [I<file>] ...>
...
Each of the I<files> is compressed according to the model selected (1,
2, 3, or I<config>) and placed in a separate segment within a single
block. The model is stored in the block header to allow decompression
later.

I<config> means to use a custom compression algorithm described in
...
The models 1, 2, and 3 select built in configurations equivalent to
I<fast>, I<mid>, and I<max> respectively, which are also available as
...
containing C<../>). If filenames are stored with backslashes (as is
customary for Windows), then they are converted to forward slashes.
...
Syntax for B<r> command is:

  I<config>[,I<arg>] ... [I<arguments>] ...

The B<r> command runs C<config.cfg> (with possible numeric I<arg>s) as
a stand-alone program. This is useful for debugging config files. The
command takes two additional arguments, I<input>, defaulting to stdin,
and I<output>, defaulting to stdout. Specifically, the HCOMP section
of the config file is run once for each byte of I<input> with any OUT
instructions writing to I<output>. Output is to the file named in the
second argument. If run with no arguments then take input from stdin
and output to stdout. See Modifiers section.
...
Compress F<file> to F<file.zpaq> using compression level 2 without
storing the filename. F<file> is not deleted:

    zpaq nc2 file

Compress files from the directory I<directory> using fast compression
and create archive F<archive.zpaq>. The filenames are stored simply as
B<file1> and B<file2> in block 1.

    zpaq c1 archive directory/file**
...
they were added block by block:
...
with a path (directory), then that directory must exist when the file
is extracted. zpaq will not create directories.
...
To extracts files, but without post-processing. This may be useful for
debugging or developing config files:

    zpaq t archive.zpaq [files ...]

List the contents of C<archive.zpaq>. The files will be displayed by
block basis:

Extract all files and put in F</tmp>.

    zpaq x archive.zpaq /tmp/

Extract the contents of the first file in block 2 to the file C<foo>.
Optimize for faster extraction.

    zpaq ox2 archive.zpaq foo

=head2 Trace

Trace (single step) the PCOMP section of I<min.cfg> with arguments 8,2
twice, first with input 3 (display decimal) and then with input 255
(display hexadecimal).

    zpaq ptrmin,8,2 3 xff
